...
class HelpForm(Form):
	...
	dob_day = IntegerField('dob_day', validators=[DataRequired()])
	dob_year = IntegerField('dob_year', validators=[DataRequired()])
	company = StringField('company', validators=[DataRequired(message='Please enter the name of your employer')])
	comment = TextAreaField('comment', validators=[DataRequired(message='Please enter a detailed comment')])
	dob = {}
	
	# This is a custom validator to check that the birthdate (three separate fields) are valid
	def validate(self):
		result = True
		if not Form.validate(self): 
			# If the form is invalidated, and the date fields are empty, add the DataRequired message to dob 
			...
			result = False  # not valid regardless of empty birthdate because the Form is invalidated
		
		# test if the birthdate is valid
		try:
			user_dob = datetime.datetime.strptime(str(self.dob_month.data) + "/" + str(self.dob_day.data) + "/" + str(self.dob_year.data), "%m/%d/%Y").date()
		except ValueError:
			self.dob['errors'].append('Please enter a valid date of birth')
